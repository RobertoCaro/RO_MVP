...
# ========================= CONFIG =========================
# RUTA FIJA Y SEGURA PARA SIEMPRE (2025 approved)
# AUTO-DETECCIÓN del último best.pt (¡NUNCA MÁS FALLA!)
def obtener_ultimo_modelo():
    # Sube un nivel (salir de Detect_image) y entra a TrainIA_image
    carpeta_runs = Path(__file__).parent.parent / "TrainIA_image" / "runs" / "detect"
    
    if not carpeta_runs.exists():
        raise FileNotFoundError(f"No se encontró la carpeta de entrenamientos:\n{carpeta_runs}")
    
    modelos = list(carpeta_runs.rglob("best.pt"))
    if not modelos:
        raise FileNotFoundError("No se encontró ningún best.pt → ¿Entrenaste el modelo?")
    
    ultimo = max(modelos, key=lambda p: p.stat().st_mtime)
    print(f"[YOLO] Modelo más reciente encontrado: {ultimo.name}")
    print(f"     → Entrenamiento: {ultimo.parent.parent.name}")
    print(f"     → Ruta: {ultimo}")
    return ultimo
